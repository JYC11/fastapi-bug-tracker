...
async def test_create_user_handler(
    ...
    async with uow:
        ...
        assert found_user, len(found_user) == 1
...
async def test_update_user_with_handlers(
    ...
    update_cmd = commands.UpdateUser(**update_data)
    update_cmd.id = user.id
    updated_user = await handlers.update_user(update_cmd, uow=uow, hasher=password_hasher)
    assert updated_user
    async with uow:
        found_user: Users | None = await uow.users.get(user.id)
        found_events1: list[EventStore] = await uow.event_store.get(user.id)
        assert found_user is not None
        assert found_user.username == update_data["username"]
        assert found_user.email == update_data["email"]
    ...
    async with uow:
        deleted_user: Users | None = await uow.users.get(user.id)
        found_events2: list[EventStore] = await uow.event_store.get(user.id)
        assert deleted_user is not None
        assert deleted_user.user_status == enums.RecordStatusEnum.DELETED
